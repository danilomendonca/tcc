...
Web Services surgiram anteriormente ao SOC, oferecendo a possibilidade de uma integração transparente entre aplicações distribuídas com o uso de um sistema de transporte de mensagens que faz uso de protocolos padronizados e diversas especificações. Em sua primeira geração, tentativas foram feitas com o uso de RPC ou Remote Procedure Calls. Apesar desse modelo já ser amplamente conhecido e usado, seu uso em Web Services não obteve apoio devido ao acoplamento resultante de configurações específicas para cada linguagem e logo deixou de ser praticado. Também foram desenvolvidos as especificações centrais, tais quais o WSDL, liguagem responsável pela descrição das funcionalidades de um Web Service, o UDDI, que consiste num registro de negócios e serviços, onde é possível encontrar (descobrir) Web Services e registra-los, além da adoção do SOAP, protocolo de troca de mensagens em formato XML e constituído sobre protocolos de transporte conhecidos da internet, i.e. HTTP e SMTP, ou mesmo outros protocolos independentes.  
