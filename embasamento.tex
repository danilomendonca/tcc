...
\subsection{Standardized Service Contract}
...
Um contrato de serviços tem a finalidade de intermediar a relação entre as partes, separando e desacoplando a descrição das funcionalidades da sua implementação. O objetivo de padronizar tais contratos está na premissa da reusabilidade, uma vez que as interfaces farão o papel de conectar um serviço a diferentes consumidores, reduzindo a necessidade de transformações de dados através do uso de suas representações unificadas. Tal medida também aumenta a interoperabilidade entre os serviços com contratos padronizados.

A composição dos contratos pode ser feita com um ou mais artefatos. Tratando-se de \textit{big web services} \cite{Pautasso:2008:RWS:1367497.1367606}, três documentos são utilizados. O WSDL descreve as funcionalidades do serviço, bem como a natureza e tipo dos dados envolvidos. Desta forma, sua padronização impacta diretamente no reuso de tais operações, evitando também sua duplicação em outros serviços. Dada sua estrutura em XML, é preciso haver esquemas que definam os modelos de dados utilizados pela descrição do serviço. Isto é possível com o uso de documentos \textit{XML schemas}. A padronização de tais esquemas possibilita a desejável redução na conversão de dados, uma vez que operações que retornem o tipo de dados poderão compartilhar um mesmo \textit{XML schema}. Por último, é preciso definir quais políticas serão utilizadas pelos serviços, função realizada pelos documentos de definição de políticas, ou \textit{WS-Policy}. 

A principal exigência do princípio em questão está na definição das funcionalidades desempenhadas pelo serviço, assim como a descrição de modelos e tipos de dados utilizados e possíveis políticas adotadas. 

%definindo SOA 
\subsection{Service Oriented Architecture}

Os princípios adotados pelo service oriented design se confundem com aqueles adotados na arquitetura orientada a objetos. Dado o peso e complexidade dessa arquitetura, a aplicação escalonável desses princípios à realidade de cada organização se faz necessária de modo a evitar um colapso da estrutura existente e os altos custos envolvidos nas fases iniciais da adoção.
...
\begin{figure}[hbtp]
	...
	\caption{Principais atributos de QoS na visão de partes de \textit{Stakeholders}. Adaptado de \cite{Balfagih:2009:10.1109}.}	
