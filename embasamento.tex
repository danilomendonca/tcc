...
%!!substituir figura por uma em português
\begin{figure}[htbp]
	\centering	\includegraphics[width=1\textwidth]{figs/danilo/Origins_SOC.eps}
...
O principal resultado da aplicação do design estabelecido pelo SOC está na criação de unidades lógicas denominadas serviços.  Assim, em vez de aplicações isoladas que trazem consigo todas as funcionalidades, são criadas unidades coesas, granulares, abstratas e de baixo acoplamento cujo acesso é feito por uma interface que encapsula a lógica e cujo funcionamento é bem descrito e garantido por um contrato. Com isso, é possível realizar uma composição ágil que irá servir uma ou, idealmente, várias aplicações por meio do reuso e da orquestração dos serviços. 
%ok! 1.1.0

Vários princípios de design regem a confecção dos serviços e de um inventório de serviços. \textit{Standard Service Contract} define que os mesmos possuam um contrato que siga padrões para definir suas funcionalidades e modelos de dados de forma a garantir a reusabilidade, a interoperabilidade e reduzir esforços de conversão de tipos de dados, além de melhorar a compreensão das capacidades dos serviços por seus gestores. O \textit{Service Loose Coupling} trata da dependência entre o contrato de um serviço, sua implementação e seus consumidores. 
